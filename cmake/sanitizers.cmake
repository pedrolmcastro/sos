...
function(sos_enable_sanitizers)
    ...
    if(MSVC)
        if(SOS_SANITIZE_LEAK OR SOS_SANITIZE_MEMORY OR SOS_SANITIZE_THREAD OR SOS_SANITIZE_UNDEFINED)
            message(AUTHOR_WARNING "MSVC doesn't support leak, memory, thread and undefined behavior sanitizers")
        endif()
    ...
    elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES ".*Clang")
        ...
        if(SOS_SANITIZE_MEMORY)
            if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
                message(AUTHOR_WARNING "GCC doesn't support memory sanitizer")
            elseif(SOS_SANITIZE_LEAK OR SOS_SANITIZE_THREAD OR SOS_SANITIZE_ADDRESS)
                message(AUTHOR_WARNING "Memory sanitizer is incompatible with leak, thread and address sanitizers")
            else()
        ...
        if(SOS_SANITIZE_THREAD)
            if(SOS_SANITIZE_LEAK OR SOS_SANITIZE_MEMORY OR SOS_SANITIZE_ADDRESS)
                message(AUTHOR_WARNING "Thread sanitizer is incompatible with leak, memory and address sanitizers")
            else()
